...
export class AllTask implements OnInit {
  ...
  visibleTasks = computed(() => {
    ...
    const children =
      new Map<number | null, TaskResponse[]>();
  ...
  rootTasks(): TaskResponse[] {
    ...
    return visible.filter(
      task =>
        ...
        !visibleIds.has(task.parentTaskId)
    );
  }
  ...
  canViewTasks(): boolean {
    return (
      this.auth.isMasterAdmin() ||
      this.auth.canWriteUsers() ||
      this.auth.canReadUsers()
    );
  }
  ...
  loadUsers(): void {
    this.http.get<AssignableUser[]>(
      ...
      next: response => {
        this.assignableUsers.set(response.filter(user => user.isDeleted !== true));
      },
  ...
  getSelectedAssigneeNames(): string[] {
    return this.getSelectedAssignees()
      .map(user => user.name);
  }
